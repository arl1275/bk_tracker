...
export let get_all_factiras_in_null_state_service = async (req: Request, res : Response) =>{
    try{
        connDB.query('SELECT * FROM facturas WHERE id_consolidado IS NULL', (err, result) => {
            if (err) {
...
export let insert_data_for_test = async (req: Request, res : Response) =>{
    let response;
    const insertquery = 'INSERT INTO facturas(ref_factura, cliente, cant_cajas, cant_unidades, lista_empaque, ubicaciones) VALUES($1, $2, $3, $4, $5, $6)';
